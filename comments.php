...
<li <?php comment_class(); ?> id="li-comment-<?php comment_ID(); ?>">
  <article id="comment-<?php comment_ID(); ?>" class="comment" role="article">
    <footer>
      <div class="comment-author vcard"> <?php echo get_avatar( $comment, 40 ); ?> <?php printf( __( '%s <span class="says">says:</span>', 'themename' ), sprintf( '<cite class="fn">%s</cite>', get_comment_author_link() ) ); ?> </div>
...
?>
  <div id="comments">
    ...
    <div class="nopassword">
      <?php _e( 'This post is password protected. Enter the password to view any comments.', 'themename' ); ?>
    </div>
  </div>
  <!-- .comments -->
  <?php return;
  ...
  <nav id="comment-nav-above" role="article">
    <h1 class="section-heading">
      <?php _e( 'Comment navigation', 'themename' ); ?>
    </h1>
